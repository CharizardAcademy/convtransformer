...
class ConvTransformerModel(FairseqEncoderDecoderModel):
    ...
    Args:
        ...
        decoder (ConvTransformerDecoder): the decoder
    ...
    def build_model(cls, args, task):
        ...
        encoder = cls.build_encoder(args, src_dict, encoder_embed_tokens)
        decoder = cls.build_decoder(args, tgt_dict, decoder_embed_tokens)
        return ConvTransformerModel(encoder, decoder)
    ...
    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return ConvTransformerDecoder(args, tgt_dict, embed_tokens)
...
class ConvTransformerDecoder(FairseqIncrementalDecoder):
    """
    Transformer decoder consisting of *args.decoder_layers* layers. Each layer
    is a :class:`TransformerDecoderLayencoder_padding_masker`.
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            ConvTransformerDecoderLayer(args, no_encoder_attn)
            for _ in range(args.decoder_layers)
        ])
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        """Upgrade a (possibly old) state dicCUDA_VISIBLE_DEVICES=1 python generate.py data-bin/wmt2015.deen/ --path checkpoints/en-de-base/checkpoint_best.pt --beam 4 --batch-size 64 --lenpen 0.6 --max-len-a 1 --max-len-b 50|tee generate.outt for new versions of fairseq."""
        if isinstance(self.embed_positions, SinusoidalPositionalEmbedding):
            weights_key = '{}.embed_positions.weights'.format(name)
            if weights_key in state_dict:
                del state_dict[weights_key]
        ...
        for i in range(len(self.layers)):
            ...
            for old, new in layer_norm_map.items():
                for m in ('weight', 'bias'):
                    k = '{}.layers.{}.layer_norms.{}.{}'.format(name, i, old, m)
                    if k in state_dict:
                        state_dict['{}.layersCUDA_VISIBLE_DEVICES=1 python generate.py data-bin/wmt2015.deen/ --path checkpoints/en-de-base/checkpoint_best.pt --beam 4 --batch-size 64 --lenpen 0.6 --max-len-a 1 --max-len-b 50|tee generate.out.{}.{}.{}'.format(name, i, new, m)] = state_dict[k]
                        del state_dict[k]
...
class ConvTransformerEncoderLayer(nn.Module):
    ...
    In the original paper each operation (mulCUDA_VISIBLE_DEVICES=1 python generate.py data-bin/wmt2015.deen/ --path checkpoints/en-de-base/checkpoint_best.pt --beam 4 --batch-size 64 --lenpen 0.6 --max-len-a 1 --max-len-b 50|tee generate.outti-head attention or FFN) is
    postprocessed with: `dropout -> add residual -> layernorm`. In the
    tensor2tensor code they suggest that learning is more robust when
    preprocessing each layer with layernorm and postprocessing with:
    `dropout -> add residual`. We default to the approach in the paper, but the
