...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        self.conv = nn.Conv1d(in_channels=self.embed_dim, out_channels=self.embed_dim, kernel_size=(self.context_size, ), padding=(self.context_size-1)//2)
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        x_self_attn, _ = self.self_attn(query=x_norm, key=x_norm, value=x_norm, key_padding_mask=None)
        x_self_attn_dropout = F.dropout(x_self_attn, p=self.dropout, training=self.training)

        x_backtrace_attn, _ = self.self_attn(query=x_norm, key=x_org, value=x_org, key_padding_mask=None)
